<?php 
	...
	//$sql = "SELECT school_id,school_name, school_contactname, DATE_FORMAT(school_enddate,'%d-%m-%Y') ,school_email,if(school_status = 2, CONCAT('DECLINED'),CONCAT('NEW')), CONCAT('<a href=newrenewal.php?action=edit&school_id=',school_id,'>Renew & Approve</a>') as action FROM wyzeup_schools WHERE school_isdeleted = 0 AND (school_status = 0 OR school_status = 2)";
	//DATE_FORMAT(t3.school_expirydate,'%d-%m-%Y'),
	$sql="SELECT  t3.school_id,t3.school_name, t3.school_contactname, 
	 		t3.school_email, if(t1.renewal_isapproved = 1, CONCAT('NEW'),CONCAT('ERROR')),
			CONCAT('<a href=renewals.php?action=edit&school_id=',t1.renewal_school_id,'>Approve Renewal</a>')  as action 
			  FROM wyzeup_renewals as t1 
			  INNER JOIN wyzeup_schools as t3 ON t3.school_id=t1.renewal_school_id  AND t1.renewal_isapproved = 1
			  WHERE t3.school_isdeleted=0 AND t1.renewal_isdeleted = 0";
	$_SESSION['ex3']=$sql;
?> 
<html>
<head>
      ...
      <tr>
        <td valign="top">
			<table width="95%" border="0" align="center" cellpadding="4" cellspacing="1">
              <tr>
                     <td width="25%" height="20"><h1 class="h1"> List of School Renewals </h1></td>
