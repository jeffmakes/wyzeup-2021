...
/**
  *
  *  PORTIONS OF THIS FILE ARE BASED ON MOVTABLE 0.91
  *
  *   Page : movtableinc.js
...
/**
  *
  *  Merged by Matt Brown (dowdybrown@yahoo.com)
  *  May 2006
  *
  *  Integrated movTable with Rico LiveGrid (merged code into a single file)
  *     Resizable columns
  *     Hide/show columns
  *     Filtering
  *  Enabled dynamic table sizing (accepts <rowcount> tag in xmlhttp response)
  *  Changed header click detection (for sorting) to be in the style of movTable
  *  Enabled basic compatibility with Opera 9b
...
var gridMenu = new Rico.Menu("18em");
...
Rico.LiveGrid.prototype = {
  ...
  initialize: function( tableId, visibleRows, totalRows, url, options, ajaxOptions ) {
    ...
    this.options = {
      ...
      allowColMove     : false, /* allow user to reorder columns - not implemented yet! */
      ...
      menuEvent        : 'dblclick',   /* event that triggers menus - click, dblclick, or null (no menus) */
      frozenColumns    : 0,
  ...
  createTables: function(visibleRows,hdrtableId) {
    ...
    for (r=0; r<headerRows.length; r++) {
      ...
      for (c=0; c<headerCells.length; c++)
        this.hdrSpan[r][c]=headerCells[c].colSpan;
  ...
  sizeDivs: function() {
    ...
    this.scrollDiv.style.height=(dataHt+this.options.scrollBarWidth)+'px';
  ...
  updateHeightDiv: function() {
    ...
    if (window.opera) ht+=this.options.scrollBarWidth-3;
  ...
  rowMouseOver: function(e) {
    ...
    this.lastHighlightRow=r.rowIndex;
    for(var t=0; t<2; t++) {
      this.tabs[t].rows[this.lastHighlightRow].style.backgroundColor=this.options.rowHighlightColor;
      //this.tabs[t].rows[this.lastHighlightRow].className='ricoRowHighlight';  // too slow!
    }
  },
  ...
  rowUnhighlight: function(rowNum) {
    ...
    for(var t=0; t<2; t++) {
      ...
      //this.tabs[t].rows[rowNum].className='';
    }
  ...
  fetchBuffer: function(offset) {
    ...
    queryString+= 'id='+this.tableId+'&page_size='+fetchSize+'&offset='+bufferStartPos;
  ...
  pluginScroll: function() {
     ...
     this.writeDebugMsg("pluginScroll");
  ...
  moveRelative: function(relOffset) {
     ...
     newoffset=Math.min(newoffset,this.scrollDiv.offsetHeight);
...
Rico.TableCell.prototype = {
  ...
  setActiveCell: function() {
    ...
    this.cells[1-this.column.tabIdx].style.display='none';
  },
  ...
  formatValue: function() {
    ...
    switch (this.column.format.type) {
      case 'raw':
        return this.value;
      case 'text':
        switch (this.column.format.markup) {
          case 'stripTags':
            /* remove all markup */
          ...
          default:
            alert("invalid value for 'markup' in column "+this.column.index);
            return this.value;
        }
      case 'number':
        return this.value.formatNumber(this.column.format);
      case 'date':
        return this.value.formatDate(this.column.format);
      case 'lookup':
        var desc=this.column.lookupValues[this.value];
        return (typeof desc=='undefined') ? this.value : desc;
      default:
        alert("invalid value for 'type' in column "+this.column.index);
        return this.value;
    }
  }
...
Rico.TableColumn.prototype = {
  ...
  handleMouseDown: function(e) {
    ...
    if (this.tabIdx>0) this.rtEdge+=this.liveGrid.tabs[0].offsetWidth;
